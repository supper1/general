                ...
                getInfoByOpenId(urlObj.open_id,cityId) // 通过openid去获取用户数据
                    .then(logins => {
                        if (logins && logins.code === 1) {
                           ...
                           window.location.href = window.location.origin+ "?cityId=" +cityId // 跳转到首页
                        }
                        resolve()
                    })
            })
...
export async function configShare() { // 验证微信分享
    ...
    wx.ready(() => {
        let url = window.location.origin + '/?cityId=' + store.getState().home.userdata.cityId
        ...
        wx.onMenuShareTimeline({
            ...
            imgUrl: 'http://xueersiimg.xrspy.com/general/icon.png', // 分享图标
            success: function () {
                // 用户点击了分享后执行的回调函数 
        ...
        wx.onMenuShareAppMessage({
            title: '《将军爷爷讲故事》德育系列访谈课正式发布', // 分享标题
            desc: '以德育树人，固教育之本，七位共和国将军亲述德育故事', // 分享描述
            link: url, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
            imgUrl: 'http://xueersiimg.xrspy.com/general/icon.png', // 分享图标
